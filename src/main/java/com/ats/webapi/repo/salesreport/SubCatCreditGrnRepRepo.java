...
public interface SubCatCreditGrnRepRepo extends JpaRepository<SubCatCreditGrnRep, Integer> {
	...
	@Query(value = "     SELECT UUID()as id, SUM(td.grn_gvn_amt) AS var_amt, SUM(td.grn_gvn_qty) AS var_qty    ,sc.sub_cat_id,sc.sub_cat_name,c.cat_id FROM t_credit_note_header tb, t_credit_note_details  td  ,m_cat_sub sc ,m_category c,m_item i  WHERE    tb.crn_id=td.crn_id AND tb.crn_date BETWEEN  :fromDate  AND  :toDate  AND td.cat_id=c.cat_id AND i.id=td.item_id AND i.item_grp2=sc.sub_cat_id AND td.is_grn=0 AND td.cat_id != 5   GROUP BY i.item_grp2  UNION    SELECT   UUID()as id,   SUM(td.grn_gvn_amt) AS var_amt,SUM(td.grn_gvn_qty) AS var_qty,4, 'Specila Cake',5 FROM t_credit_note_header tb, t_credit_note_details  td     WHERE tb.crn_id=td.crn_id AND tb.crn_date BETWEEN  :fromDate  AND  :toDate   AND td.is_grn=0 AND td.cat_id = 5  GROUP BY td.cat_id", nativeQuery = true)
	...
	@Query(value = "     SELECT UUID()as id,SUM(td.grn_gvn_amt) AS var_amt, SUM(td.grn_gvn_qty) AS var_qty    ,sc.sub_cat_id,sc.sub_cat_name,c.cat_id FROM t_credit_note_header tb, t_credit_note_details  td  ,m_cat_sub sc ,m_category c,m_item i  WHERE    tb.crn_id=td.crn_id AND tb.crn_date BETWEEN  :fromDate  AND  :toDate  AND td.cat_id=c.cat_id AND i.id=td.item_id AND i.item_grp2=sc.sub_cat_id AND td.is_grn=0 AND td.cat_id != 5 AND tb.fr_id=:frId GROUP BY i.item_grp2  UNION    SELECT   UUID()as id,   SUM(td.grn_gvn_amt) AS var_amt,SUM(td.grn_gvn_qty) AS var_qty,4, 'Specila Cake',5 FROM t_credit_note_header tb, t_credit_note_details  td     WHERE tb.crn_id=td.crn_id AND tb.crn_date BETWEEN  :fromDate  AND  :toDate   AND td.is_grn=0 AND td.cat_id = 5 and tb.fr_id=:frId GROUP BY td.cat_id", nativeQuery = true)
